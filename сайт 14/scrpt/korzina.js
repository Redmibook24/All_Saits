...
// Делаем корзину доступной для "drop"
cartElement.addEventListener("dragover", function(event) {
    event.preventDefault();
});

// Обработчик для добавления товаров в корзину при перетаскивании
cartElement.addEventListener("drop", function(event) {
    event.preventDefault();
    const productName = event.dataTransfer.getData("text/plain");
    addToCart(productName);
});

// Устанавливаем обработчик для каждой карточки товара
...
// Изначальное отображение корзины
updateCartDisplay();
